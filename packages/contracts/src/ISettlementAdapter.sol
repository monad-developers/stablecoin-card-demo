...
/// @dev A holder approves the adapter over `asset()`; the configured `issuer()` pulls funds
///      at settlement. One adapter is deployed per strategy and shared by every holder — the
///      holder's wallet *is* the account.
interface ISettlementAdapter {
    ...
    /// @notice The token a holder approves this adapter to pull (equals `stablecoin()` for 1:1).
    function asset() external view returns (address);

    /// @notice Spendable value for `holder`, in `stablecoin()` base units.
    /// @dev Bounded by the holder's `asset()` balance and their allowance to this adapter.
    ...
    /// @dev Only callable by `issuer()`. Pulls `asset()` from `holder` via allowance,
